...
frumvarp_data = []
frumvarp_insert = []
with open('data/nyttfrumvarp.txt', 'r') as infile:
	for line in infile.readlines():
		#stjornarskra_data.append(line.split())
		print(line, expr.split(line)[1])
...
#insert sentances
#c.executemany('insert into compare_sentence (origin, text, article_nr) values (?,?,?)', stjornarskra_insert)
#c.executemany('insert into compare_sentence (origin, text, article_nr) values (?,?,?)', frumvarp_insert)
#conn.commit()

#insert articles
#article_nrs = list(set(c.execute('select article_nr from compare_sentence where origin=1').fetchall()))
...
#for row in articles:
#	for article_id in c.execute('select id from compare_sentence where article_nr=?', (row[1],)).fetchall():
#		c.execute('insert into compare_article_sentences (article_id, sentence_id) values (?,?)', (row[0], article_id[0]))
#conn.commit()
